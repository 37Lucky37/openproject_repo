pipeline {
    ...
    environment {
        ...
        WORKSPACE_DIR = "${HOME}/openproject" 
            ...
            steps {
                script {
                    sh """
                        ...
                        echo '⬇Завантажуємо середовище RVM...'
                        echo 'source \$HOME/.rvm/scripts/rvm' >> ~/.bashrc

                        echo '⬇Встановлюємо Ruby...'
                ...
                script {
                    sh '''
                        ...
                        if [ -z "$USER_EXISTS" ]; then
                            ...
                            echo "Користувач вже існує. Пропускаємо."
                        fi
                        ...
                        if [ -z "$DB_EXISTS" ]; then
                            ...
                            echo "База даних вже існує. Пропускаємо."
                        fi
                    '''
                }
            }
...
EOL
        ...
        stage('Install Node.js') {
            steps {
                script {
                    sh """
                        echo 'Встановлюємо Node.js ${NODE_VERSION}...'
                ...
                script {
                    sh """
                        ...
                        if [ -f package.json ]; then
                            ...
                            echo ' package.json не знайдено. Пропускаємо встановлення.'
                        fi
                    """
                }
            }
        ...
        stage('Check Environment') {
            steps {
                script {
                    sh """
                        echo ' Перевіряємо середовище на агенті:'
                        uname -a
                        whoami
                        pwd
                    """
